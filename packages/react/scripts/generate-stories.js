...
function analyzeProps(content, componentName) {
  ...
  const interfaceRegex = new RegExp(
    `interface\\s+${componentName}Props\\s*(?:extends[^{]*)?{([^}]*)}`,
    "s"
  );
    ...
    if (match) {
      ...
      while ((paramMatch = paramRegex.exec(paramsContent)) !== null) {
        ...
        if (propName && propName !== "children") {
          ...
          if (propName.includes("count") || propName.includes("size") || propName.includes("index"))
            control = "number";
...
function generateStoryContent(componentName, fileInfo, props) {
  ...
  const variantStories = Array.from(uniqueVariantStories.entries())
    .map(
      ([storyName, { prop, option }]) => `
...
${defaultArgs
  .split("\n")
  .filter(line => !line.includes(`${prop}:`))
  .join("\n")}`
        : ""
    }
  },
...
function generateStories() {
  ...
  files.forEach(fileInfo => {
    ...
    components.forEach(componentName => {
      ...
      const storyDirPath =
        fileInfo.dirPath === "." ? STORIES_DIR : path.join(STORIES_DIR, fileInfo.dirPath);
